'''
Initially copied from https://github.com/thecml/mensa/blob/main/src/sota_models.py
I added ConvNet at the beginning of each SOTA model to allow for feature extraction from fixed-length time-series data.
'''
...
class DeepSurv(nn.Module):
    ...
    def calculate_baseline_survival(self, x, t, e):
        ...
        return self.time_bins, self.cum_baseline_azhard, self.baseline_survival 
...
def train_deepsurv_model(
    ...
    train_loader = DataLoader(TensorDataset(x_train, t_train, e_train), batch_size=config.batch_size, shuffle=True)
