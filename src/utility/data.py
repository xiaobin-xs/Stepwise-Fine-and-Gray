...
def prepare_surv_label_for_sksurv(df, event_col='event_indicator', time_col='time_to_event', event_id=1):
    return np.array([(True if event==event_id else False, duration) 
                        for event, duration in zip(df[event_col], df[time_col])], dtype=[('event', '?'), ('time', '<f8')])
